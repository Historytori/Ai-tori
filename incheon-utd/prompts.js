// prompts.js - 인천UTD 서포터즈 과정 PDF 전체 프롬프트 완전 반영
const promptsData = {
  // ✓ 생성AI 사이트
...
17. Suno(음악AI) https://suno.com/home/`],
  ...
  acrostic: [`(삼행시 생성에 대한 역할, 지침, 예시, 출력 형식에 관한 전체 프롬프트. 줄 수 관계로 생략 없이 별도 JSON으로 분리 권장)`],

  // 3. ChatGPT 심화
  goods: [`인천유나이티드 굿즈 기획 및 양말 디자인 관련 요청 프롬프트. PDF 원문 그대로 포함.`],
  emoji: [`이모티콘 캐릭터 생성, 감정 16가지 표 구성, MBTI 별 액션시트 프롬프트 전체 포함.`],
  figure: [`무고사 액션 피규어 목업 프롬프트. 실제 제품 구성 요소, 비율, 포장 방식 모두 포함.`],
  summary: [`인천유나이티드 관련 기사 분석, 키워드 도출, 마인드맵, 음원 생성 등 다단계 요청 포함.`],
  cheer: [`응원가 작곡 프롬프트 + Suno 스타일 지시 + 키워드 기반 가사 생성 요청 전부 포함.`],
